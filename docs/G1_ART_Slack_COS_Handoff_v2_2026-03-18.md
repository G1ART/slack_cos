...
- **구현 (2026-03-28 갱신)**: `app.js` `handleUserText` 는 **`runInboundCommandRouter`** (`src/features/runInboundCommandRouter.js`) 호출 후 미스 시 **`runInboundAiRouter`** 만 이어 붙인다. pre-AI 순서: 도움말 → **`tryFinalizeProjectIntakeCancel`** → **활성 인테이크 → `tryFinalizeProjectSpecBuildThread`** (`projectSpecSession.js`: 조회·M4·플래너 락 **양보·명시 Council 연기·그 외 spec mutation / `project_spec_execution_ready`|`project_spec_refine`**) → **결정 짧은 회신** → transcript 기반 **`start_project` 잠금·정제**(인테이크 비활성/null 시)·**Front Door** → **M4 lineage** → **`tryFinalizeSlackQueryRoute`** → 동기 `routing_sync_*` → 플래너 하드 락 → **`runInboundStructuredCommands`** → **대표 surface**. AI 꼬리: 조회 → 인테이크 취소 → **동일 `tryFinalizeProjectSpecBuildThread`(전체 경로)** → `tryStartProjectLockConfirmedResponse` 등 → 내비 → planner 방화벽 → **명시 Council·인테이크 연기** → **`runCouncilMode`** → **`dialog`**. 상세: `COS_Inbound_Routing_Current_260323.md`.
...
- **코드**:
  - `src/features/runInboundCommandRouter.js` — pre-AI 파이프라인(도움말·**인테이크 취소**·**활성 인테이크 `tryFinalizeProjectSpecBuildThread`**·결정 짧은 회신·transcript `start_project` 루프·M4 lineage·조회·`routing_sync_*`·플래너 하드 락·`runInboundStructuredCommands`·**구조화 `finalizeSlackResponse`**·surface)
  - `src/features/runInboundStructuredCommands.js` — 구조화 명령 대량 분기(미스 시 `undefined`)
  - `src/features/runInboundAiRouter.js` — `runInboundAiRouter`, `classifyInboundResponderPreview`(도움말·인테이크 취소·**`tryFinalizeProjectSpecBuildThread`(previewOnly)**·transcript **`start_project_confirmed`/`start_project_refine`**·Front Door·조회·…·내비/Council/dialog)
  - `src/features/projectSpecSession.js` · `src/features/projectSpecModel.js` — **인테이크 빌드 스레드 정본(spec)**·`computeSufficiency`·`project_spec_execution_ready` / `project_spec_refine`
  - `src/features/scopeSufficiency.js` · `src/features/startProjectLockConfirmed.js` — transcript 경로: **`assessScopeSufficiency`**·`start_project_confirmed` / `start_project_refine`(인테이크 비활성·또는 spec 핸들러 양보 시)
...
- **(2026-04-01) Founder launch gate**: `src/core/founderLaunchGate.js` 등 — direct chat에서 launch 문구 감지 시 파트너 LLM 대신 readiness·provider truth·execution spine 연결; `scripts/test-founder-launch-gate.mjs`(`npm test`). **(2026-04-03 vNext.13.2)** 창업자 면 텍스트는 `src/founder/founderLaunchFormatter.js`·`founderLaunchApprovalPacket.js`만 사용 (`founderRenderer`·`evaluatePolicy` 없음). trace `surface_type`은 여전히 `EXECUTION_PACKET` / `LAUNCH_BLOCKED`. 회귀 추가: `test-vnext13-2-launch-gate-purification.mjs`.
- **(2026-04-01) Project space thread-first**: `projectSpaceBootstrap.js` — fuzzy 라벨 점수 재사용 제거, exact 라벨/alias + 격리 규칙만 재사용; registry에 `goal_fingerprint`·`last_*`·`spaceHasActiveRunOnOtherThread`; launch trace에 `project_space_resolution_mode` 등; `scripts/test-project-space-bootstrap-hardening.mjs`.
...
- **(2026-04-06) vNext.13.12 Root surgery (ChatGPT in Slack)**: 창업자 기본 경로 **transcript·durable state 미주입**; **현재 턴 첨부 메타**만; **founder assistant 미버퍼**; **thin salvage-first**; **state 파일 기본 repo 밖** (`COS_RUNTIME_STATE_DIR`). 상세: `docs/cursor-handoffs/COS_vNext13_12_Root_Surgery_ChatGPT_In_Slack_2026-04-06.md`.
