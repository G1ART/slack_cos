...
- **(2026-04-01) Founder launch gate**: `src/core/founderLaunchGate.js` 등 — direct chat에서 launch 문구 감지 시 파트너 LLM 대신 readiness·provider truth·`EXECUTION_PACKET` / `LAUNCH_BLOCKED`·execution spine 연결; `scripts/test-founder-launch-gate.mjs`(`npm test`). **`EXECUTION_PACKET` 렌더**는 목표/범위/스페이스/run/provider truth/수동 브리지/기본값/next action을 본문에 고정(`founderRenderer` ↔ `executionLaunchPacketBuilder`).
