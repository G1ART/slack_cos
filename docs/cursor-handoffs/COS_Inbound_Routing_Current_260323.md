...
10. `runInboundAiRouter` — `classifyInboundResponderPreview`: 도움말 다음 **`start_project_confirmed`** → **`start_project_refine`** → **Front Door** → …; AI 꼬리·내비 본문도 동일 순서. **명시 Council** 시 **`isStartProjectKickoffInput`**·**`isStartProjectLockConfirmedContext`**·**`isStartProjectRefineFlowContext`**·**`resolveCleanStartProjectKickoff`** 이면 `upsertApprovalRecord` 생략 / dialog  
