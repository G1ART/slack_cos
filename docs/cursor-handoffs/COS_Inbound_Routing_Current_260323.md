...
보강8: founder route 판정은 `source_type` 단일 값에 의존하지 않는다. `slack_route_label`(`dm_ai_router`/`mention_ai_router`) 및 DM 채널 키(`D...`)까지 포함해 판정한다. vNext.13.3: 조건식은 **`src/founder/founderRouteInvariant.js` SSOT** 한 곳이며 `app.js`·`runInboundAiRouter` 가드가 이를 호출한다.
...
보강26 (2026-04-03, vNext.13): 창업자 자연어 기본 표면은 **`proposal_packet_surface`** (`founderProposalKernel` + 선택적 `runCosNaturalPartner` 보강). 외부 디스패치는 `external_execution_authorization` 승인 게이트 통과 후 `ensureExecutionRunDispatched`에서만 진행. launch gate는 **`launchMinimalWorkContext`**로 work_object 파서 없이 스레드·인테이크·런만 본다.
보강27 (2026-04-04, vNext.13.4): 창업자 단일 경로는 **`runFounderDirectKernel`** (`founderDirectKernel.js`). 흐름: durable state 로드 → **`planFounderConversationTurn`**(mock / `callJSON` / 파트너 폴백) → state persist → **`tryArtifactGatedExecutionSpine`** — 유효한 **`execution_artifact`**만 **`runFounderLaunchPipelineCore`**로 실행 스파인 연결. 원문만으로 launch 금지. **운영 메타 좁은 락**: `classifyFounderRoutingLock`·`classifyFounderOperationalProbe`에 걸리면 커널 입구에서 `tryResolveFounderDeterministicUtility`(+ `founder_explicit_meta_utility_path` 동등)로 **`RUNTIME_META`**·trace `founder_operational_meta_short_circuit` — 대화 플래너 미경유. 회귀: `test-vnext13-4-founder-*.mjs`, 갱신된 `test-founder-launch-gate.mjs`·`test-founder-operational-probe.mjs`·`test-vnext13-3-founder-ambiguous-intent.mjs`(`npm test`).
보강28 (2026-04-04, vNext.13.5): 보강19의 **`maybeHandleFounderLaunchGate`**·**`founderLaunchIntent.js` 원문 런치** 전제는 **폐기**. 창업자 면은 **`handleUserText` → `runFounderDirectKernel`** 만 (command·AI 라우터 미도달). launch 권한은 **`tryArtifactGatedExecutionSpine`** + **`validateExecutionArtifactForSpine`**(durable lineage) 통과 시 **`runFounderLaunchPipelineCore`** 만. raw-text intent는 **`src/legacy/founderLaunchIntentRawText.js`**·**`maybeHandleFounderLaunchGateRawText`** **회귀 전용** (`src/core`·`src/founder`·`app.js` 에서 legacy import 금지). **운영 메타 숏서킷**은 **`metadata.founder_explicit_meta_utility_path === true`** 일 때만. trace: **`founder_staging_mode`**·**`founder_preflight_boundary`** (`COS_FOUNDER_STAGING_MODE=0` 제외). 회귀: **`scripts/test-vnext13-5-*.mjs`**.
보강29 (2026-04-04, vNext.13.5b): spine eligibility 는 **`convStateBeforeTurn` persisted lineage** 만 — **`evaluateExecutionSpineEligibility`**·**`buildPersistedEligibleLineageView`**. 같은 턴 sidecar `state_delta`/artifact id 병합분은 persist 후보만이며 **현재 턴 실행 근거 불가**; 우회 시도 시 trace `founder_spine_eligibility_reason` (`same_turn_lineage_not_eligible` 등). 회귀: **`test-vnext13-5b-*.mjs`**, 갱신 **`test-vnext13-5-approval-lineage-crosscheck.mjs`**. 레거시 launch 스모크: **`npm run test:legacy-launch-regression`**.
보강30 (2026-04-01, vNext.13.6, **13.7 갱신**): **`registerHandlers`** 창업자 DM·멘션에서 `extractFilesFromEvent` → **`founderIngestSlackFilesWithState`**(`ingestSlackFile`·DOCX·PDF·PNG+vision·페이로드 시그니처) → 스레드 문서 버퍼·**`latest_file_contexts`**. **vNext.13.7**: 실패는 플래너 `userText`에 넣지 않고 **`partner_natural_surface`** 짧은 안내만; 성공만 요약 컨텍스트로 `handleUserText`에 합류. 상세: **`COS_vNext13_6_…`** · **`COS_vNext13_7_…`**.
보강31 (2026-04-05, vNext.13.7): 창업자 **기본 Slack 표면**은 **`partner_natural_surface`**(자연어). **`[COS 제안 패킷]`** 전개는 **외부 실행 후보(`external_execution_tasks`)가 있을 때** 승인 블록만 병합. `sendFounderResponse` 가 금지 마커(패킷·페르소나 잔재)를 최후에 차단. 회귀: **`scripts/test-vnext13-7-*.mjs`**.
보강32 (2026-04-01, vNext.13.11): **`planFounderConversationTurn` 의 `callJSON` 구조화 플래너**는 기본 **비활성**(`COS_FOUNDER_STRUCTURED_PLANNER=1` 또는 `metadata.enableFounderStructuredPlanner` 일 때만). 기본 턴은 **`founder_chat_only`**(빈 sidecar) → 슬랙 본문은 **`runCosNaturalPartner` 1회**만(이중 LLM·플래너 NL 혼선 제거). **`route: null`** 이면 파트너 시스템 프롬프트는 **짧은 ChatGPT형 COS** 분기. mock 플래너 행은 기존과 동일 우선. 상세: **`docs/cursor-handoffs/COS_vNext13_11_Founder_Chat_First_Planner_Opt_In_2026-04-01.md`**.
보강33 (2026-04-06, vNext.13.10): **운영 창업자 DM/멘션**은 **`runFounderDirectKernel` → `runFounderNaturalChatOnly`** — `planFounderConversationTurn`·**`callJSON` 없음**; **`app.js` 는 창업자 커널에 `callJSON` 메타를 넣지 않음**. 구조화 플래너·아티팩트 게이트·제안 패킷 조립은 **`runFounderArtifactConversationPipeline`** (`founderArtifactConversationPipeline.js`) — **`npm test`·mock·E2E dress** 전용. 회귀 파이프에서 파트너 호출이 비어 있거나 폴백 문구만 나오면 **mock `natural_language_reply`** 로 표면을 채운다. 첨부 전부 실패도 **별도 실패-only 숏서킷 없이** 동일 `handleUserText` 경로. outbound thin: **`thinFounderSlackSurface`**.
보강34 (2026-04-06, vNext.13.12): 창업자 기본 경로는 **`getConversationTranscript` / `mergeFounderConversationState` / `synthesizeFounderContext` 미사용**; **`priorTranscript: ''`**. 인제스트 결과는 **`current_attachment_contexts`·`current_attachment_failures`** 로만 본문 조립. **`recordInboundSlackExchange`**: `metadata.founder_route === true` 이면 **assistant 턴 미기록**. **`founderConversationState` 파일 기본 경로**: repo 밖 tmp (`resolveFounderConversationStatePath`). **`thinFounderSlackSurface`**: `sanitizeFounderOutput` 기반 salvage-first.

보강25 (2026-04-02): **창업자 DM은 Council 미호출** — `app.js` `handleUserText` 가 `founderRoute` 이면 `founderRequestPipeline` 만 타고 `runInboundAiRouter`·Council 실행 경로로는 내려가지 않는다. `PARTNER_NATURAL` 은 `runCosNaturalPartner`(단일 LLM)만 쓰며, 모델이 위원회 메모 포맷을 흉내 내면 **`sanitizeFounderOutput`**(`founderRequestPipeline` → `founderSurfaceGuard.js`)으로 정리한 뒤 전송한다. 회귀: `scripts/test-partner-natural-sanitize.mjs`(`npm test`).
보강24 (2026-04-02): **창업자 direct chat 운영 질문** — `COS_FOUNDER_DIRECT_CHAT=1` 이면 launch gate 다음·파트너 LLM 직전에 `classifyFounderOperationalProbe`(`inboundFounderRoutingLock.js`)가 **Git SHA/커밋 질문**·**Cursor Cloud 브리지 상태**·**Supabase 연결(코스 관점)** 을 매칭하면 `formatRuntimeMetaSurfaceText` 또는 `buildProviderTruthSnapshot` 기반 `RUNTIME_META` 표면으로 **결정론 응답**한다. `looksLikeRuntimeShaQuery`는 헌법 파이프라인의 `버전` 잠금(`classifyFounderRoutingLock` → `RUNTIME_META`)과 동기화. 회귀: `scripts/test-founder-operational-probe.mjs`(`npm test`).
보강23 (2026-04-02, 갱신): **Provider truth / outbound 정렬** — `buildProviderTruthSnapshot`: Cursor는 **`cursor_trace`에 live 디스패치 성공**이면 `live`; **`COS_CURSOR_CLOUD_LAUNCH_URL`만 구성**(아직 디스패치 전)이면 `live_ready`; **핸드오프 파일/트레이스**만 있으면 `manual_bridge`; 둘 다 없으면 `unavailable`. Supabase는 **`supabase_trace`에 `execution_tier: live` + dispatched**면 `live`; **live dispatch URL 구성**(기본 production 직접 apply 비활성, `COS_SUPABASE_SAFE_TARGET`)이면 `live_ready`; 프로젝트 연결·스키마 드래프트·migration 스텁·dispatch 실패 이력 등만 있으면 `draft_only`; 미연결이면 `not_configured`. `formatProviderTruthFriendlyLines` → 패킷 필드 `provider_truth_friendly`가 **실행 패킷**(`renderExecutionPacket`)·**진행 보고**(`renderStatusReportPacket`)·**핸드오프**(`renderHandoffPacket`)에서 동일 모델로 노출된다. `founderRequestPipeline.executeSpine`·상태 조회 분기도 동일 스냅샷에서 friendly 줄을 채운다. 오케스트레이션: `dispatchOutboundActionsForRun` → `ensureCursorOutboundForRun`·`tryEnsureSupabaseLiveOrDraftForRun`. 실행 패킷에 `provider_native_refs`·`auto_started_artifacts`(GitHub/Cursor/Supabase 네이티브 ref)가 실린다. 회귀: `scripts/test-live-provider-truth-alignment.mjs`(live_ready가 draft_only로 뭉개지지 않음 등)·`scripts/test-execution-outbound.mjs`(`npm test`).
...
| `src/core/founderRequestPipeline.js` | **오퍼레이터·채널 면** — 창업자 `founder_route` 는 `app.js`에서 **`runFounderDirectKernel`** 로 먼저 return 하므로 본 파이프라인에 도달하지 않음. Constitution: 유틸 → 조회/구조화 `null` → phase·실행기·골드. |
| `src/core/founderLaunchGate.js` | **`runFounderLaunchPipelineCore`** — artifact-gated launch 및 legacy 회귀 `trace_tags` 전용; 프로덕션에서 raw-text intent import 없음. 창업자 표면은 **`src/founder/founderLaunchFormatter.js`** 등. 회귀: `test-founder-launch-gate.mjs`, `test-vnext13-5-*.mjs`. |
| `src/legacy/founderLaunchIntentRawText.js` | **회귀 전용** raw-text launch 문구 감지 — 프로덕션 트리 import 금지 (`test-vnext13-5-no-production-import-of-legacy-launch.mjs`). |
