...
**코드 기준일**: 2026-03-27 (surface `start_project`: `tryClassifyStartProject` — 멀티라인·`G1COS`/줄 앞 잡음 허용; 코어: `surfaceIntentClassifier.js`) · 원본 2026-03-23  
...
3. **M4 lineage drill-down** (`tryFinalizeG1CosLineageTransport`) — 한 줄: `턴 <uuid>` / `추적` / `trace` · `패킷 PKT-…` · **`상태 STP-…`** / `status STP-…`(M2b `status-packets.jsonl` 감사) · `워크큐 AWQ-…` · **`워크큐 목록` / `워크큐 대기`**(최근·`pending_executive`+`queued`) · **`실행 큐 목록` / `고객 피드백 목록`**(`cos-workspace-queue.json`) · **`실행 큐 CWS-…` / `고객 피드백 CFB-…`** 드릴다운 → `inbound-turn-trace.jsonl`·감사 JSONL(결정·상태)·`agent-work-queue.json`·워크스페이스 큐 JSON **읽기 전용** (`responder: query`, `response_type: lineage_*`). 워크큐 응답에 **`queued`/`pending_executive` → `커서발행`/`이슈발행`/`수파베이스발행` 실행 브리지**·`in_progress` 시 **`워크큐증거`/`러너증거`**·목록 푸터 **CI 훅(`COS_CI_HOOK_*`)** 안내·**`proof_refs` 길이 요약**(과도한 줄 truncate). **조회(PLN/WRK 한 줄)보다 앞**.
4. 조회 전용 (`tryFinalizeSlackQueryRoute`)  
5. 플래너 하드 락  
6. `runInboundStructuredCommands` (내부 실행 어휘) — **직전**에 `start_project` 이면 **`pre_structured_start_project`** 로 즉시 `tryExecutiveSurfaceResponse` 반환(구조화 오탐·지연 제거). 포함: **`워크큐*`**(`AWQ-*`, `data/agent-work-queue.json`) · **`실행큐계획화`/`실행큐계획`** `CWS-…` 또는 **인자 없음 / `최근`/`latest`** → 가장 늦게 쌓인 미승격 spec 큐를 PLN·WRK로 승격(`workspaceQueuePromote.js`). **`커서발행`·`이슈발행`/`깃허브발행`·`수파베이스발행` 성공 시** 동일 WRK 활성 `AWQ-*`에 **`linkAgentWorkQueueRunForWork`** → `linked_run_id` 또는 증거-only **`dispatch_run:<RUN>`** · **`COS_AGENT_BRIDGE_URL`** 설정 시 성공 직후 **`agentBridgeOutbound.js`** 가 외부 URL로 **`tool_dispatch`** JSON POST(fire-and-forget). **증거만**: `워크큐증거`·`러너증거`(`proof_refs` append, 상태 유지). **옵션 HTTP**: `COS_CI_HOOK_PORT` + `COS_CI_HOOK_SECRET` 설정 시 `app.js`가 **`GET /cos/health`**(무인증)·**`POST /cos/ci-proof`**(`src/runtime/ciWebhookServer.js`)로 증거 append.  
7. **Surface intent** (`tryExecutiveSurfaceResponse`) — 예: `결정비교:` → 얇은 결정 패킷 Slack 텍스트 + (메타 있을 때) 감사 `decision-packets.jsonl` append·스레드 tail 저장; **`전략 검토:`·`리스크 검토:`** 등 v0 가이드 응답; **`ask_status`** 는 **`executiveStatusRollup.js`** 로 AWQ·PLN·WRK·**실행 큐(`spec_intake`)** 로컬 스토어 집계를 상태 패킷 본문에 합성(v1); **`start_project`** (`프로젝트시작`/`툴시작`/`툴제작` 등, `classifySurfaceIntent` / **`tryClassifyStartProject`**: 접두 + **빌드 시그널** `tryClassifyStartProjectByBuildSignals` — `만들자`+캘린더/앱 등, `MVP`+도메인, 짧은 FAQ 문장은 제외) — 첫 줄 인사·`G1COS`/`COS`/`비서` 접두·`툴제작:` 단독 줄+후속 텍스트 인식. **첫 응답 계약**은 `startProjectSurfaceCopy.js`: 이해·기본 가정·기술 확인 2~3개·다음 산출물(실행 큐/PLN·WRK); **초기 턴에 APR 생성 안 함**(문구로 명시). (Slack **metadata** 있을 때) **`spec_intake` 실행 큐 append** + `실행큐계획화` 안내; 환경 변수 **`COS_FAST_SPEC_PROMOTE=1`**(로컬·데모)이면 같은 응답에 **`workspaceQueuePromote`**(PLN·WRK) 블록까지 붙임; **`product_feedback`** (`피드백:`/…)는 (메타 있을 때) **`customerFeedbackAwqBridge`** — CFB + **`feedback_follow_up` AWQ 초안** · `approvalMatrixStub` `customer_feedback_intake` 티어 · `linked_awq_id`; 구조화 **`고객피드백:`**·자연어 피드백 인테이크도 동일 브리지; **`hold_pause`·`request_deploy_readiness`** 등은 각각 `response_type`·finalize `command_name`·trace `surface_intent` 에 동일 라벨(패킷만 `decision_packet`). 다각은 `협의모드:`.  
8. `runInboundAiRouter` — 내비(본문이 `start_project` 이면 LLM 내비보다 **`tryExecutiveSurfaceResponse` 우선**) / planner 방화벽 / 비명시 턴에서 `start_project` 이면 **`routeTask` 호출 전**에 surface로 귀환(`ai_tail_start_project_guard`) / **명시 Council** 시 **`isStartProjectKickoffInput`** 이면 `upsertApprovalRecord` 생략 / dialog  
