...
보강30 (2026-04-01, vNext.13.6): **`registerHandlers`** 창업자 DM·멘션에서 `extractFilesFromEvent` → **`ingestSlackFile`**(DOCX·PDF·PNG+vision) → 스레드 문서 버퍼(`addDocumentToThread`)와 **`mergeFounderConversationState`**의 **`latest_file_contexts`**(실패 행도 기록). `handleUserText` 입력 앞에 **파일 인테이크 프리앰블**을 붙여 실행·승인과 분리된 인테이크임을 표시. 상세: **`docs/cursor-handoffs/COS_vNext13_6_Slack_File_Intake_Founder_DM_2026-04-01.md`**.
