...
**코드 기준일**: 2026-03-28 (**`start_project` 2턴 잠금** — `startProjectLockConfirmed.js`·킥오프 COS 직후·확정 시 `start_project_confirmed`; **Front Door**·M4 lineage 순) · 원본 2026-03-23  
...
3. **`start_project` 범위 잠금(2턴)** (`tryStartProjectLockConfirmedResponse` / `startProjectLockConfirmed.js`) — 바로 **직전 COS 턴**이 킥오프 정렬(`*[정렬 · 툴/프로젝트 킥오프]*` 등)이고, 현재 사용자 메시지에 **확정 시그널**(진행해줘·좋아 등)+충분한 답변(또는 짧은 확정 한 줄)이면 **킥오프 Front Door보다 먼저** 잠금·실행 전환 표면(`start_project_confirmed`)으로 종료. Council·APR·`업무등록:` 유도 없음; `spec_intake` 큐에 `[project_lock_confirmed]` 남김.
...
10. `runInboundAiRouter` — `classifyInboundResponderPreview`: 도움말 다음 **`start_project_lock_confirmed`** → **Front Door** → …; AI 꼬리에서도 동일. **명시 Council** 시 **`isStartProjectKickoffInput`**·**`isStartProjectLockConfirmedContext`**·**`resolveCleanStartProjectKickoff`** 이면 `upsertApprovalRecord` 생략 / dialog  
