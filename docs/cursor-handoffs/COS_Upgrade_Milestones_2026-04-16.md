...
- **W1 Tool plane (클로즈아웃):** `dispatchExternalToolCall.js` 는 공통 진입·검증·레지스트리 `getLaneAdapter`·차단/프리트리거·`runCursorCloudAutomationExecutionBranch` 위임까지만 유지; Cursor 클라우드/emit_patch/콜백·로컬 CLI·artifact 폴백 본문은 `toolPlane/lanes/cursor/cursorCloudAutomationPath.js` 및 `cursorEarlyExitBlocks.js`·`cursorEmitPatchAssemblyBlock.js`·`cursorDelegateMerge.js`·`cursorOrchestratorStatusMap.js` 로 분리. `toolsBridge.js` 는 호환 facade; `externalToolLaneRegistry.js` 는 런타임 바인딩 SSOT. Founder 측은 `founderSystemInstructions.js` / `founderConversationInput.js` / `founderToolLoop.js` + 얇은 `runFounderDirectConversation.js`. 구조 테스트에 `test-w1-dispatch-cursor-split-boundary.mjs` 추가. orchestrator_toolplane 프리플라이트: `npm run preflight:orchestrator_toolplane` · `npm run verify:preflight:orchestrator_toolplane` (매니페스트/ack 파일명 `w1_remaining_g3_ab500d7`, 로컬 `ops/` 생성·gitignore).
