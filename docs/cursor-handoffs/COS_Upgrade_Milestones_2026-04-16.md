...
- **W1 Tool plane (클로즈아웃):** `dispatchExternalToolCall.js` 가 외부 툴 런타임 SSOT(레지스트리 `getLaneAdapter`·레인 모듈 `toolPlane/lanes/*`); `toolsBridge.js` 는 호환용 얇은 facade; `externalToolLaneRegistry.js` 는 laneKey·adapter·readiness·invocationPrecheck 바인딩; founder 측은 `founderSystemInstructions.js` / `founderConversationInput.js` / `founderToolLoop.js` + 얇은 `runFounderDirectConversation.js`; 구조 고정 테스트 `test-runFounderDirectConversation-is-thin-coordinator`·`test-external-tool-lane-registry-is-runtime-ssot`·`test-dispatchExternalToolCall-uses-lane-registry`·`test-no-broad-toolsBridge-imports-outside-approved-compat`. Preflight: `ops/preflight_manifest/w1_closeout_5024db1.json` + `ops/preflight_ack/w1_closeout_5024db1_ack.json` (task `w1_closeout_5024db1`, workstream `orchestrator_toolplane`).
