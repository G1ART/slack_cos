...
**`.env` 에 뭘 넣을지·우선순위·추천:** `COS_Tenancy_Keys_And_Env_Guide_2026-04-15.md`.
...
- **6 (cos_runs):** `cos_runs` 네 테넄시 컬럼 + `applyCosRunTenancyDefaults` / `appRunToDbRow` (`20260416130000_*`). 요약·감사 CLI에 `--workspace-key` 등; `summarize-ops-smoke-sessions` 는 **Supabase 모드에서만** `COS_PARCEL_DEPLOYMENT_KEY` env 기본 필터 (file·memory JSONL은 `--parcel-deployment-key` 명시).
- **요약 final_status (후속):** `aggregateSmokeSessionProgress` — `authoritative_callback_closure_applied` 가 있으면 뒤에 오는 `callback_correlated_but_closure_not_applied` 로 `final_status` 를 덮어쓰지 않음 (중복 웹훅·live_37 유형).
...
- **Phase 1 + 3 (초기):** `COS_Phase1_CrossLayer_Envelope_2026-04-15.md` 봉투 필드 표; `cosSlackAppIdentity.js` · `COS_SLACK_APP_ID` · 부트 `cos_runtime_truth.slack_app_id` (선택).

## 다음 단계 (즉시)

1. **Phase 1** 스케치: **`COS_Phase1_CrossLayer_Envelope_2026-04-15.md`** 에 봉투 필드 SSOT 초안 고정 (run/packet/테넄시/founder vs audit 구분).  
2. **3번** 확장점: **`cosSlackAppIdentity.js`** + env `COS_SLACK_APP_ID` + 부트 `cos_runtime_truth` 노출 — 멀티 앱 분기 앵커. (프로세스당 Bolt 하나는 유지.)  
3. 운영 Supabase에 `20260414120000`·`20260415120000`·`20260416130000` 마이그레이션 적용 후 `audit:parcel-health`·요약으로 **deployment** 스코프 검증; 새 런 insert 시 `cos_runs` 테넄시 컬럼 채움 확인.  
4. 테넄시 키 **ledger·기타 비스트림** 경로 우선순위를 founder와 찍는다 (`cos_runs`·요약 CLI는 반영됨).
...
- 출시·재배포: `COS_Release_Readiness_Checklist_2026-04-16.md`.  
- 업그레이드 마일스톤(M0~M5): `COS_Upgrade_Milestones_2026-04-16.md`.  
- 제품 서술·외부 로드맵: `WHAT_WE_ARE_BUILDING_G1_COS_2026-04-14.md`, `G1_COS_Upgrade_Roadmap_2026-04-14.md`.
