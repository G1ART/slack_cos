...
2. **GitHub repo live create** — repos.create API 구현 (permissions 확보 시)
3. **Vercel/Railway live deploy** — 실제 API 호출 배포 지원
4. **Supabase CLI auto-apply** — `supabase db push` 자동화
5. **Project space 목록 조회** — 대표가 "내 프로젝트 목록" Slack에서 조회
6. **Approval response routing** — 대표 승인/거부 응답 → run state 자동 전이
