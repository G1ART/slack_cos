...
## 17. v1.1 Production Deployment Hardening (2026-03-31)

### 진단: Railway 배포 미반영

Slack smoke 테스트에서 `@G1COS 버전`이 **old Council 포맷**(종합 추천안, 페르소나별 핵심 관점, 내부 처리 정보)으로 응답. `origin/main` HEAD는 kernel swap sha `68cf956`이나, **Railway 프로덕션이 이 커밋을 반영하지 않은 상태**임을 확인.

### 근본 원인

1. `app.js` → `runLegacySingleFlow()` → `reportComposer.js:composeFinalReport()` — OLD 포맷("한 줄 요약", "추천안", "내부 처리 정보")을 직접 생성하는 legacy path
2. OLD `council.js` `synthesizeCouncil()` — "종합 추천안", "페르소나별 핵심 관점", "참여 페르소나:" 등 생성
3. OLD `registerHandlers.js` — `sendFounderResponse` 대신 직접 `say()`/`replyInThread()` 사용 → hard-block 우회

### 수정 사항

| 파일 | 변경 |
|---|---|
| `app.js` | pipeline 호출에 try/catch + diagnostic logging 추가. pipeline error 시 legacy fallthrough. startup 시 `pipeline_loaded` 확인 로그 |
| `src/agents/reportComposer.js` | `composeFinalReport` 완전 재작성 — old markers ("한 줄 요약", "추천안", "내부 처리 정보", "승인 대기열") 모두 제거. 새 포맷: `*요약*`, `*COS 권고*`, `*주요 반론*`, `*리스크*`, `*다음 행동*`, `*대표 결정 필요*` |

### npm test 결과

전체 통과 (constitutional 209 assertions 포함).

---

## 18. Next Patch Priorities

1. **Railway 재배포 확인** — `@G1COS 버전`에서 sha 확인, `pipeline_loaded=true` 로그 확인
2. **Golden path live 검증** — 실제 Slack thread에서 kickoff→lock→execute→approve→deploy 전체 경로 pipeline 통과 확인
3. **Legacy router freeze** — command/AI router에서 pipeline 처리 가능한 경로 점진 이관 (structured commands + query 렌더러)
