...
- **창업자 (`founderRoute`)**: `app.js` 첫 블록에서 `runFounderDirectKernel`만 — **command/AI router 미진입** (회귀: `test-vnext12-1-founder-no-command-router.mjs`, `test-vnext13-founder-no-routing-surface.mjs`). Launch gate 통과 시 표면은 `founderLaunchFormatter.js` 전용 (vNext.13.2). 제안 턴에 한해 `cosGovernanceAdvisory` **선택 부록** 가능 (라우터 아님).
